...
export function BinaryLab() {
  ...
  return (
    <section className="learning-section" id="bits">
      <div className="section-kicker"><span>03</span><p>情報量とその単位</p></div>
      <div className="section-title-row"><div><p className="step-label">組み合わせる</p><h2>8個のスイッチで何通り？</h2></div><p className="section-question">1桁増えると、表せる情報はどう変わるだろう。</p></div>
      <div className="lab-panel bit-lab">
        <div className="bit-switches" aria-label="8ビットスイッチ">
          {bits.map((bit, index) => <button type="button" key={index} className={bit ? 'on' : ''} aria-pressed={Boolean(bit)} aria-label={`${128 >> index}の位、現在${bit}`} onClick={() => toggle(index)}><span>{128 >> index}</span><b>{bit}</b><small>2<sup>{7 - index}</sup></small></button>)}
        </div>
        <div className="number-readout">
          <div><span>2進数</span><strong>{bits.join('')}</strong></div>
          <div><span>10進数</span><strong>{decimal}</strong></div>
          <div><span>16進数</span><strong>{decimal.toString(16).toUpperCase().padStart(2, '0')}</strong></div>
        </div>
