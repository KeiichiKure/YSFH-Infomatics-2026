...
const terms = [
  ['1', '圧縮', '情報をできるだけ保ちながら、ファイルサイズを小さくする処理'],
  ['1', '冗長', '同じ情報や不要な部分が含まれている状態'],
  ['1', '可逆圧縮', '伸張すると圧縮前と同一のデータへ戻る圧縮'],
  ['1', '非可逆圧縮', '重要度の低い情報も省き、完全には元へ戻らない圧縮'],
  ['2', 'エンコード', '元のデータを決められた方法で変換すること'],
  ['2', 'デコード', '変換されたデータを利用できる形へ戻すこと'],
  ['2', 'ZIP', '文書やプログラムなどに使われる代表的な可逆圧縮形式'],
  ['2', 'JPEG', '色数の多い写真に適した非可逆圧縮形式'],
  ['3', 'ランレングス圧縮', '同じデータの連続回数を記録する方法'],
  ['3', 'LZ圧縮', '過去に現れた位置と長さで同じ並びを表す方法'],
  ['3', 'LZW圧縮', '出現した文字列を辞書へ登録し、番号で表す方法'],
  ['3', 'ハフマン圧縮', '出現頻度が高いデータへ短い符号を与える方法'],
  ['3', 'キーフレーム', '動画で一定の間隔ごとに全体を保存したフレーム'],
  ['4', 'BMP', 'すべての画素を保存する非圧縮の画像形式'],
  ['4', 'GIF', 'おもに256色以下の画像に向く可逆圧縮形式'],
  ['4', '256色', 'GIFが主に扱う色数の上限'],
  ['4', 'JPEG', '写真に向き、境界線の表現が苦手な非可逆圧縮形式'],
  ['4', 'PNG', '可逆圧縮でフルカラーや透明を扱える画像形式'],
];
...
export function CompressionCheckpoint() {
  ...
  return (
    <section className="learning-section checkpoint" id="checkpoint-02-03">
      ...
      <div className="section-title-row"><div><p className="step-label">言葉と判断をつなぐ</p><h2>重要語を体験から説明しよう</h2></div><p className="section-question">緑の番号はプリントの設問番号ではなく、このページの単元1〜4です。カードを確認したら、3つの保存場面に答えよう。</p></div>
      <div className="term-grid">{terms.map(([unit, term, meaning], index) => <details key={`${unit}-${term}-${index}`}><summary><span aria-label={`単元${unit}`}>{unit}</span><strong>{term}</strong></summary><p>{meaning}</p></details>)}</div>
