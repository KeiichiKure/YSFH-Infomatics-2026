...
type LzwDecodeStep = { code: number; text: string; result: string; action: string; added?: { code: number; text: string } };
...
function decodeLzw(output: number[], initial: { code: number; text: string }[]) {
  ...
  for (const code of output) {
    ...
    result += text;
    let added: { code: number; text: string } | undefined;
    if (previous && text) {
      added = { code: dictionary.size + 1, text: previous + text[0] };
      dictionary.set(added.code, added.text);
    }
    const action = isNewCode
      ? `番号${code}は今から登録される番号。直前の「${previous}」に先頭の「${previous[0]}」を足して「${text}」と読める。`
      : `番号${code}を辞書で調べると「${text}」。復元結果の後ろへ付ける。`;
    steps.push({ code, text, result, action, added });
    previous = text;
  }
  return steps;
}
...
const huffmanRows = [
  ['ま', 4, '0'], ['な', 3, '10'], ['ご', 2, '110'], ['む', 1, '11100'],
  ['ぎ', 1, '11101'], ['め', 1, '11110'], ['た', 1, '11111'],
] as const;
const huffmanAssignmentReasons = [
  '最も多い「ま」へ、最短の1 bit「0」を割り当てる。',
  '「00」「01」は0を読んだ時点で「ま」と決まるため使えない。次に短い2 bitの「10」を「な」へ割り当てる。',
  '「100」「101」は先頭の10で「な」と決まるため使えない。残る枝の「110」を「ご」へ割り当てる。',
  '1回の文字は、まだ文字が決まらない「111」の枝を使い、「む」を11100にする。',
  '同じ111の枝で末尾が重ならないように、「ぎ」を11101にする。',
  '続いて空いている「11110」を「め」へ割り当てる。',
  '最後の「た」を「11111」へ。どの符号も別の符号の先頭部分にならない表が完成する。',
];
...
export function LosslessLab() {
  ...
  const decodedHuffmanText = huffmanChars.slice(0, huffmanDecodeStep + 1).join('');
  ...
  const isFullFrame = frame === 1 || frame === 8;
    ...
    <section className="learning-section" id="lossless">
      ...
      <div className="lossless-workbench">
        ...
        {method === 'lzw' && <div className="method-panel" role="tabpanel">
          ...
          <div className="decode-lab">
            ...
            <div className="output-code-row" aria-label="LZWで復元する番号列">{lzw.output.map((code, index) => <i className={index === lzwDecodeStep ? 'is-current' : index < lzwDecodeStep ? 'is-read' : ''} key={index}>{code}</i>)}</div>
            <div className="step-card"><span>復元 {lzwDecodeStep + 1} / {lzwDecoded.length}</span><strong>{currentLzwDecodeStep.action}</strong><p>番号 <code>{currentLzwDecodeStep.code}</code> → 文字列 <code>{currentLzwDecodeStep.text}</code></p></div>
            ...
            <div className="decode-result"><span>ここまでの復元結果</span><strong>{currentLzwDecodeStep.result}</strong><small>{currentLzwDecodeStep.added ? `同時に辞書へ ${currentLzwDecodeStep.added.code}＝${currentLzwDecodeStep.added.text} を追加` : '最初の番号を初期辞書で読みました'}</small></div>
          </div>
        ...
        {method === 'huffman' && <div className="method-panel" role="tabpanel">
          ...
          {huffmanPhase === 'assign' && <div className="step-experience">
            ...
            <div className="prefix-rule"><span>なぜ0の次が10なの？</span><div><code>0</code><p><b>「ま」で確定。</b>00や01は、最初の0だけで「ま」と読まれてしまいます。</p></div><div><code>10</code><p><b>「な」で確定。</b>ほかの符号は10から始めません。</p></div><small>完成した表では、どの符号も別の符号の先頭部分になりません。これを接頭語条件といいます。</small></div>
          </div>}
          ...
          {huffmanPhase === 'decode' && <div className="step-experience huffman-stepper">
            ...
            <div className="step-card"><span>復元 {huffmanDecodeStep + 1} / {huffmanChars.length}</span><strong>{currentDecodedHuffmanCode === '0' ? '先頭に0が出たので、1 bitだけで「ま」と決まる。' : `先頭の1ではまだ決まらない。${currentDecodedHuffmanCode}まで読むと「${currentDecodedHuffmanChar}」に決まる。`}</strong></div>
            <div className="bit-reading" aria-label="ハフマン符号を1ビットずつ読む">{Array.from(currentDecodedHuffmanCode).map((bit, index) => { const prefix = currentDecodedHuffmanCode.slice(0, index + 1); const decided = index === currentDecodedHuffmanCode.length - 1; return <div className={decided ? 'is-decided' : ''} key={prefix}><b>{bit}</b><span>{prefix}</span><small>{decided ? `「${currentDecodedHuffmanChar}」に決定` : 'まだ続く'}</small></div>; })}</div>
            <input aria-label="ハフマンで復元する手順" type="range" min="0" max={huffmanChars.length - 1} value={huffmanDecodeStep} onChange={(event) => setHuffmanDecodeStep(Number(event.target.value))} />
            <div className="step-buttons"><button type="button" disabled={huffmanDecodeStep === 0} onClick={() => setHuffmanDecodeStep((value) => Math.max(0, value - 1))}>← 前へ</button><button type="button" disabled={huffmanDecodeStep >= huffmanChars.length - 1} onClick={() => setHuffmanDecodeStep((value) => Math.min(huffmanChars.length - 1, value + 1))}>次へ →</button></div>
      ...
      <div className="keyframe-lab">
        ...
        <div className="keyframe-compare">
          <div className={`mini-frame ${isEvening ? 'is-evening' : ''}`}><span>{frame === 1 ? 'キーフレーム：昼の全体' : frame === 8 ? '新しいキーフレーム：夕方の全体' : `再生時の画面 ${frame}`}</span><i className="mini-house" /><i className="mini-tree" /><i className="mini-person" style={{ left: `${personLeft}%` }} />{birdVisible && <i className="mini-bird" style={{ left: `${birdLeft}%`, top: `${20 + (frame % 3) * 4}%` }} />}</div>
          <i aria-hidden="true">→</i>
          <div className={`mini-frame difference frame-${frame} ${frame === 8 ? 'is-evening full-frame' : ''}`}><span>{isFullFrame ? `フレーム${frame}：全体を保存` : `フレーム${frame}：差分だけ`}</span>{isFullFrame && <><i className="mini-house" /><i className="mini-tree" /></>}<i className="difference-person" style={{ left: `${personLeft}%` }} />{birdVisible && <i className="mini-bird difference-bird" style={{ left: `${birdLeft}%`, top: `${20 + (frame % 3) * 4}%` }} />}<b>{frame === 1 ? '昼の背景・家・木・人を保存' : frame === 8 ? '夕方へ変わった画面全体を保存' : birdVisible ? '人と鳥の位置だけを保存' : '人の位置だけを保存'}</b></div>
        </div>
        <p className="teacher-note">フレーム1は昼の全体、フレーム2〜7は動いた人や鳥の差分を保存します。フレーム8で夕方へ大きく変わるため全体を新しいキーフレームとして保存し、フレーム9以降は再び差分だけに戻ります。</p>
      </div>
    </section>
  );
}
