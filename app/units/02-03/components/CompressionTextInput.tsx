...
export function CompressionTextInput({ method, example, activeText, isBasic, onApply }: {
  ...
  return <form className="compression-input" onSubmit={(event) => { event.preventDefault(); apply(draft); }}>
    ...
    <p id={`${id}-help`}>{method === 'lzw' ? '半角の大文字 A・B・Cのみ、1〜32文字。使う文字をA・B・Cの順に初期辞書へ登録します。' : 'ひらがな（長音「ー」も可）、32文字以内・2〜7種類。回数に偏りのある文字列を試してみよう。'} 編集中は下の手順を変えず、ボタンを押すと最初から計算します。入力は送信・保存しません。</p>
    ...
    <p className="active-input" role="status">{isBasic ? '基本例' : '自分で試す'}：<b>{activeText}</b>（{Array.from(activeText).length}文字・{new Set(activeText).size}種類）{draft.normalize('NFC') !== activeText && <em>未反映の編集があります</em>}</p>
  </form>;
}
