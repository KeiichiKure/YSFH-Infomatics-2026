...
const terms = [
  ...
  ['11', 'ピクセル', 'ラスタ画像を格子状に区切った最小の点。1画素ごとに色の数値を持つ。'],
  ['12', '光の三原色', 'ディスプレイで使う赤（R）・緑（G）・青（B）の光。3色の明るさの組合せで色を表す。'],
  ['13', '加法混色', '光を加えるほど明るくなり、赤・緑・青を最大に重ねると白になる混色。'],
  ['14', '画素数', '画像を構成する画素の数。横の画素数×縦の画素数で求め、多いほど細部を表しやすい。'],
  ['15', '解像度', '一定の長さや範囲にどれだけ細かく画素が並ぶかを表す度合い。高いほど輪郭を細かく表せる。'],
  ['16', '階調', '1色の明るさを表せる段階数。量子化ビット数がn bitなら2のn乗段階になる。'],
  ['17', '24ビットフルカラー', '赤・緑・青をそれぞれ8 bit、合計24 bitで表し、約1677万色を区別する色表現。'],
  ['18', 'フレーム', '動画を構成する1枚ずつの静止画像。フレームを順番に表示して動きを作る。'],
  ['19', 'fps', 'frames per secondの略。1秒間に表示するフレーム数で、大きいほど動きが滑らかに見える。'],
  ['20', '3DCG', '遠近法・重なり・陰影などを使い、平面の画面に立体物や奥行きを表現する技術。'],
  ['21', '視差', '左右の目に届く像の位置のずれ。脳が物体までの距離や奥行きを判断する手がかりになる。'],
];
